...
// based on observed behavior, it looks like this takes radius instead of width. May be dependent on anchor point
...
// function to deal with number sequences in JSON spritesheets. Flash exports png sequences with 4 digit suffixes, so use this to determine which one to use when looping through spritesheet frames. Useful in PIXI.js when loading movieclip textures.
...
// CURRENTLY PREFERRED METHOD FOR TESTING FOR TOUCH CAPABILITIES
...
// User Agent Sniffing method of mobile detection - not optimal
...
// testing touch capability detection - don't typically use this one for some reason :/
...
/*
    Get file type from extension. Removes query strings that come after the file request as well. Does NOT deal with invalid inputs at the moment, so should probably validate that input equals output
*/
...
// takes a jquery selector and inserts a non-breaking space character between the last two words. Useful for headlines. Commented out while unused, so as not to cause errors due to the jQuery dependency
...
/* UNTESTED!!
    In theory sets the source of the first img element to the one you want to preload. Probably not the best way of doing things, might require there to be an img element in the dom that is hidden test before using in production!!
*/
/*
